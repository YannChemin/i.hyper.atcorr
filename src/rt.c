...
/* ------------------------------------------------------------------ *
 * sixs_os: compute atmospheric path reflectance xl(-mu,1)/xmus.
 *
 *   ctx        - per-thread context (polar.betal, del.delta, quad.nquad,
 *                multi.igmax, err used)
 *   iaer_prof  - 0 = exponential profile (only supported)
 *   tamoy      - aerosol optical depth (truncated, DISCOM applies coeff)
 *   trmoy      - Rayleigh optical depth
 *   pizmoy     - truncated single scattering albedo
 *   tamoyp     - aerosol OD above observer plane
 *   trmoyp     - Rayleigh OD above observer plane
 *   palt       - observer altitude km (>900 = satellite)
 *   phirad     - relative azimuth angle (radians)
 *   nt         - number of atmospheric layers
 *   mu         - Gauss quadrature points per hemisphere
 *   np         - number of azimuth planes stored in xl
 *   nfi        - number of azimuth angles in xlphim
 *   rm_off     - Gauss cosines, size 2mu+1, rm_off[j+mu]=rm(j)
 *   gb_off     - Gauss weights, size 2mu+1, gb_off[j+mu]=gb(j)
 *   rp         - azimuth angles for xl planes [np]
 *   xl         - output radiance [(2mu+1)*np]; xl[0]=xl(-mu,1)=path refl*xmus
 *   xlphim     - path reflectance vs azimuth [nfi]
 *   rolut      - angular LUT reflectance [mu*61] (can be NULL to skip)
 * ------------------------------------------------------------------ */
