...
/* Compute molecular (Rayleigh) reflectance using Chandrasekhar's method.
 *
 * xphi : relative azimuth angle between sun and observation (degrees, 0=backscattering)
 * xmuv : cosine of the observation (view) zenith angle
 * xmus : cosine of the solar zenith angle
 * xtau : Rayleigh (molecular) optical depth
 *
 * Returns: molecular reflectance xrray (unitless, 0–1) */
