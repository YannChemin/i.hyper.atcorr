...
/* Compute direct/diffuse transmittances and spherical albedo.
 * Ports SCATRA.f exactly.
 * Outputs: ddirtt,ddiftt = downward dir/dif total
 *          udirtt,udiftt = upward dir/dif total
 *          sphalbt       = total spherical albedo
 *          ddirtr..sphalbr = Rayleigh only
 *          ddirta..sphalba = aerosol only */
...
/* ISO.f port: compute diffuse transmittance xtrans[-1,0,1] using SOS.
 * xtrans[-1] = upward diffuse transmittance at plane level (view direction)
 * xtrans[0]  = 0.5 * spherical albedo
 * xtrans[1]  = downward diffuse transmittance (total atm) */
