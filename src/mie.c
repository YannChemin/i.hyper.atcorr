...
/* ──────────────────────────────────────────────────────────────────────────
 * EXSCPHASE — Mie extinction/scattering efficiencies and phase function for
 * a single sphere at size parameter X = 2π r / λ with complex refractive
 * index m = nr - i*ni.
 *
 * Outputs:
 *   Qext, Qsca : efficiency factors
 *   p11[nbmu]  : scattering intensity at nbmu Gauss points
 *
 * cgaus_S[nbmu], pdgs_S[nbmu] : Gauss quadrature cosines and weights.
 * ipol must be 0 (scalar only — polarisation terms Q/U not computed here).
 * ────────────────────────────────────────────────────────────────────────── */
...
/* ──────────────────────────────────────────────────────────────────────────
 * sixs_mie_init — fill ctx->aer for a custom log-normal aerosol.
 *
 * Parameters (log-normal size distribution):
 *   r_mode  : mode radius (µm)
 *   sigma_g : geometric standard deviation (dimensionless, > 1)
 *   m_r_550 : real part of refractive index at 550 nm
 *   m_i_550 : imaginary part (positive, absorption)
 *
 * Uses a constant refractive index across all 20 reference wavelengths.
 * For a spectrally varying index the caller should call aerosol_init with
 * a custom mixture instead.
 * ────────────────────────────────────────────────────────────────────────── */
