 ...
 *
 * Extends sixs_os() to propagate the full Stokes vector (I,Q,U).
 * The main physical effect captured over the scalar version is Rayleigh
 * polarisation: solar intensity generates Q and U via gamma2, and the
 * multiply-scattered Q/U feed back into I (improving R_atm accuracy by
 * 1–5 % in blue bands for typical geometries).
 *
 * Aerosol simplification: gammal = zetal = 0, alphal = betal (spherical
 * particles).  gr(j,k) = gt(j,k) = 0 for aerosol.
 *
 * Memory:  three copies of i1/i2 (I,Q,U) → 6 × (nt+1) × (2mu+1) doubles.
 *          This is ~3× the scalar footprint.  Temporary arrays are heap-
 *          allocated and freed on exit. */
